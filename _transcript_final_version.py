...
PROJECT_DIR = r"D:\DrugCLIP_Project"
DOCKING_DIR = os.path.join(PROJECT_DIR, "docking")
VINA_EXE = os.path.join(DOCKING_DIR, "vina_1.2.5_win.exe")
SCREENING_RESULTS = os.path.join(PROJECT_DIR, "tcmbank_screening_results.csv")
PROTEIN_PDB = os.path.join(DOCKING_DIR, "wrn_receptor.pdb")
PROTEIN_ORIGINAL = os.path.join(DOCKING_DIR, "8pfl.pdb")
TOP_N = 3
...
for idx, row in top.iterrows():
    ...
    try:
        ...
        # 清理配体中的非标准原子（保留ROOT/ENDROOT）
        with open(ligand_pdbqt_raw, 'r') as f:
            lines = f.readlines()
        fixed_lines = []
        for line in lines:
            if line.strip().startswith(('ATOM','ROOT','ENDROOT','BRANCH','ENDBRANCH')):
